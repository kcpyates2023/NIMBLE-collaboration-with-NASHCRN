...
Table_Fun<-function(FIT){
  ...
  CI_low<-AUCROC-qnorm(0.975)*SE_AUC
  CI_upper<-AUCROC+qnorm(0.975)*SE_AUC 
  ...
  Tab_Total[["**"]]<-"**"
  Tab_Total[["Area Under the ROC Curve(95%CI)"]]<-paste0(format( round(AUCROC,3), nsmall = 3),"(", format( round(CI_low,3), nsmall = 3),",",format( round(CI_upper,3), nsmall = 3),")")
  ...
  Tab_Positive[["**"]]<-"**"
  Tab_Positive[["Area Under the ROC Curve(95%CI)"]]<-"  "
  ...
  Tab_Negative[["**"]]<-"**"
  Tab_Negative[["Area Under the ROC Curve(95%CI)"]]<-"  "
